        ...
        <div className='flex items-center flex-col w-[95vw] sm:w-[85vw] md:w-[80vw] '>
            ...
            <table className='w-full flex flex-col gap-[10px]'>
                <thead className='w-full'>
                    <tr className='flex w-full justify-around bg-[#FFD700]  rounded-md text-sm sm:text-base '>
                ...
                <tbody className='w-full'>
                    {arr?.filter((coin)=>coin.name.toLowerCase().includes(searchquery) || coin.symbol.toLowerCase().includes(searchquery)).slice(page * 10 - 10, page * 10).map((coin) => (
                        <tr onClick={() => clicked(coin.id)} className='py-3 text-xs vs:text-base w-full flex  hover:bg-zinc-600 text-[#FAF0E6] hover:font-bold hover:text-black rounded-md cursor-pointer' key={coin.id}>
            ...
            <div className='flex justify-center gap-[6px] sm:gap-[10px]  p-3 text-lg text-[#FAF0E6]'>
                <span onClick={() => setthispage(page - 1)} className={page > 1 ? "cursor-pointer flex items-center" : "opacity-0"}><FaCircleChevronLeft /></span>
                {
                    len > 0 &&
                    [...Array(len)].map((val, i) => {
                        return <span className={`${page === i + 1 ? "bg-[#FFD700] text-black" : "text-[#FAF0E6]"} cursor-pointer hover:bg-[#87CEEB] hover:text-black  rounded-full w-[20px] sm:w-[25px] flex justify-center items-center`} onClick={() => setthispage(i + 1)}>{i + 1}</span>
                    })
                }
